...
def main():
    ...
    os.mkdir(str(run_number))
    print(str(run_number))
    #we want to plot reconstructed hits and propagated hits together.
    #first, we create arrays for prop_hits and rec_hits:
    partialProphitGlobalX = t["partialProphitGlobalX"].array( entry_stop = entry_stop1)
    partialProphitEta = t["partialProphitEta"].array( entry_stop = entry_stop1)
    partialTrackChamber = t["partialTrackChamber"].array( entry_stop = entry_stop1)
    rechitGlobalX = t["rechitGlobalX"].array( entry_stop = entry_stop1)
    rechitEta = t["rechitEta"].array( entry_stop = entry_stop1)
    rechitChamber = t["rechitChamber"].array( entry_stop = entry_stop1)
    ...
    for i in range(0, 4):
        ...
        ax2.legend()
        fig.tight_layout()
        fig.savefig(run_number + "/prop_rec_hits_layer%d.png" %i)
    ...
    for i in range(0, 4):
        ...
        for j in range(1,5): 
            ...
            try:
                popt, perr = normal_fit(bin_heights, bin_borders, axs[a,b], bounds_array = ([0,-0.7, +0.3],[(3)*(max(bin_heights)), 0.7, 1.5]) )
            except Exception:
        ...
        fig.tight_layout()
        fig.savefig(run_number + "/residuals_eta_layer%d.png" %i)
    #fig_chamber.savefig(run_number + "/spatialResolutionVSeta.png")
    fig_chamber.tight_layout()
    fig_chamber.savefig(run_number + "/spatial_resolution_vs_eta.png")
    #
    #
    #
    ...
    for i in range(0, 4):
        ...
        fig.tight_layout()
        fig.savefig(run_number + "/matching_eta_ch%d.png" %i)
    #
    #
    #
